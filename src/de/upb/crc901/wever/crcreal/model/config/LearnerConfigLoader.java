...
public class LearnerConfigLoader {
	...
	private void loadPEEA() {
		final List<String> modelObjectives = ListUtil
				.commaStringToList(AllTestExampleObjective.ID + "," + RelevantPartObjective.ID);
		final List<String> passiveModelObjectives = new LinkedList<>();
		final List<AlgorithmConfig> modelAlgorithmConfigs = IntStream.range(0, 2).mapToObj(
				x -> new AlgorithmConfig(SimpleModelAlgorithm.ID, modelObjectives, passiveModelObjectives, true, 50))
	...
	private void loadEEA() {
		...
		final List<AlgorithmConfig> testAlgorithmConfigs = IntStream.range(0, 1).mapToObj(
				x -> new AlgorithmConfig(SimpleTestAlgorithm.ID, testObjectives, passiveTestObjectives, false, 100))
				.collect(Collectors.toList());

		final LearnerConfig config = new LearnerConfig("crc.real.learner.eea", true, modelAlgorithmConfigs,
	...
	private void loadPMOO() {
		final List<String> modelObjectives = ListUtil.commaStringToList(
				PositiveTestExampleObjective.ID + "," + NegativeTestExampleObjective.ID + "," + RelevantPartObjective.ID
		...
		final List<AlgorithmConfig> modelAlgorithmConfigs = IntStream.range(0, 1).mapToObj(
				x -> new AlgorithmConfig(NSGAIIModelAlgorithm.ID, modelObjectives, passiveModelObjectives, true, 100))
				.collect(Collectors.toList());
	...
	private void loadEEPMOO() {
		...
		final List<AlgorithmConfig> modelAlgorithmConfigs = IntStream.range(0, 1).mapToObj(
				...
				.collect(Collectors.toList());
		modelAlgorithmConfigs.add(
				new AlgorithmConfig(NSGAIIPreSatModelAlgorithm.ID, modelObjectives, passiveModelObjectives, true, 50));

		final List<String> testObjectives = ListUtil
				.commaStringToList(DisagreementObjective.ID + "," + MinTestLengthObjective.ID);
		final List<String> passiveTestObjectives = new LinkedList<>();
		final List<AlgorithmConfig> testAlgorithmConfigs = IntStream.range(0, 1).mapToObj(
				x -> new AlgorithmConfig(NSGAIITestAlgorithm.ID, testObjectives, passiveTestObjectives, false, 100))
				.collect(Collectors.toList());

		final LearnerConfig config = new LearnerConfig("crc.real.learner.eepmoo", true, modelAlgorithmConfigs,
				testAlgorithmConfigs);
		this.algorithmConfigList.add(config);
	}

	private void loadEEMOO() {
		// final List<String> modelObjectives =
		...
		final LearnerConfig config = new LearnerConfig("crc.real.learner.eemoo", true, modelAlgorithmConfigs,
				testAlgorithmConfigs);
		this.algorithmConfigList.add(config);
	}
